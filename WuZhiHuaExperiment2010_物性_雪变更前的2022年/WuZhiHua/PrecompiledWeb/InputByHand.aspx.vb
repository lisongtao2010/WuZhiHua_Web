...
#Region "同批次"
    ...
    Public Function ShowSamePiCi() As Boolean
        ...
        If lastCheckResultMS.Rows.Count > 0 Then
            Me.lblHint.Text &= " 新生产已入力 有检查结果!"

            '相同座番 是否有待判数据 判定

            rows = GetDatatableRows(lastCheckResultMS, "作番 = '" & Me.tbxMakeNumber.Text & "' AND 状态='待判'", "结束时间 DESC")
            ...
            '相同座番 完了数据 判定
            rows = GetDatatableRows(lastCheckResultMS, "作番 = '" & Me.tbxMakeNumber.Text & "' AND 状态='完了'", "结束时间 DESC")
            If rows.Length > 0 Then
            ...
            '不相同座番 完了数据 判定
            rows = GetDatatableRows(lastCheckResultMS, " 状态='待判'", "结束时间 DESC")
            If rows.Length > 0 Then
                'CASE1  存在待判的场合。选项：1 重新检查 2 继续检查  3 不检查
                GoTo SamePiCi
            ...
            '不相同座番 完了数据 判定
            rows = GetDatatableRows(lastCheckResultMS, " 状态='完了'", "结束时间 DESC")
            If rows.Length > 0 Then
...
SamePiCi:
        If lastCheckResultMS.Rows.Count > 0 Then
            ViewState("result_id") = lastCheckResultMS.Rows(0).Item("ID")
        End If
    ...
    Public Function ShowOldCheckResult() As Boolean
        ...
        '一览表示 同批次数据取得
        Dim lastCheckResultMS As Data.DataTable = _
        checkDA.GetLastReultInfoFrom_t_check_result(Me.tbxGoodsCd.Text, Me.tbxMakeNumber.Text, "1~2")
        ...
        If lastCheckResultMS.Rows.Count > 0 Then
            ...
            Me.lblHint.Text &= " 有检查结果（t_check_result） "
            '相同座番 是否有待判数据 判定
            Dim rows As DataRow()
            rows = GetDatatableRows(lastCheckResultMS, "作番 = '" & Me.tbxMakeNumber.Text & "' AND 状态='待判'", "结束时间 DESC")
            If rows.Length > 0 Then
        ...
        Else
            ...
            'Me.lblHint.Text &= " 新生产未入力 并且不存在检查记录"
            Me.lblHint.Text &= " 无检查结果（t_check_result） "
            GoTo SamePiCi
